...
class Three_answers_options(BaseModel):
    ...
    response: List[str] = Field(
        description="Tres respuestas tipo test de la pregunta con las letras A), B) Y C)"
    )
    correct_response: str = Field(description="El numero de la respuesta correcta")


parsers = {"three_answers": PydanticOutputParser(pydantic_object=Three_answers_options)}
